...
카메라·마이크·위치·알림을 차례로 요구하는 페이지를 만들어
`127.0.0.1:8731` 에 띄우고, `ALLY_URL` 을 그리로 두고 앱을 켰다
(127.0.0.1 은 보안 컨텍스트로 쳐주므로 `getUserMedia` 가 된다).

| 권한 | 거절 코드 **있을 때** | 거절 코드 **뺐을 때** |
|---|---|---|
| 카메라 | 거절됨 (NotAllowedError) | AbortError *(이 PC 에 카메라가 없어 판정 불가)* |
| **마이크** | 거절됨 | **허용됨** |
| **위치** | 거절됨 | **허용됨** |
| **알림** | denied | **granted** |

빼면 마이크·위치·알림을 **묻지도 않고 내준다.** 위의 이동 구멍과 겹쳤으면
남의 페이지가 마이크를 조용히 켤 수 있었다.

## 나머지 동작도 눌러 봤다

| | 결과 |
|---|---|
| 닫기(X) | 프로세스 살아 있고 창만 숨음 |
| 트레이 아이콘 클릭 | 숨은 창이 다시 나옴 |
| 두 번 실행 | 프로세스 수 그대로(4→4), 숨은 창이 앞으로 |
| 자동 시작 | 첫 실행에 `HKCU\...\Run` 에 `kr.allywork.messenger` 로 등록됨 |

## 시험할 때 조심할 것 — 자동 시작 항목이 겹친다

`app.setLoginItemSettings` 는 **AppUserModelId 를 항목 이름으로 쓴다.**
설치본과 개발본이 같은 id(`kr.allywork.messenger`)라, 개발본을 한 번
띄우면 **설치본의 자동 시작 항목이 개발본 경로로 덮인다.**

실제로 그렇게 됐고 되돌렸다. 개발본으로 시험한 뒤에는 반드시 확인할 것:
