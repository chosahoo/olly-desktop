...
윈도우에 electron 을 깔아() 개발 모드로 띄우고, 프로필을
따로 준 뒤() 눌러 봤다. 사장님이 쓰시던 설치본은
건드리지 않았다.

## 화면 밖 창 되살리기 — 부숴서 확인

화면은 1707 너비인데  에  을 심어 두고 띄웠다.
...
없는 주소로 띄우니 '연결하지 못했습니다 / 다시 시도' 가 떴다.
...
처음엔  가 뜨면 무조건 오프라인 화면을 띄웠다. 그랬더니
**바깥 링크를 눌렀을 뿐인데 앱 창이 보던 화면을 잃었다.** 우리 주소를
못 열었을 때만 띄우도록 고쳤다().

 도 바깥으로 올렸다 —  처리기가 그보다 먼저
등록되는데 안쪽 const 로 두면 나중에 순서를 건드릴 때 TDZ 로 터진다.

## CDP 로 강제 이동시키면 다르게 보인다

 로 밀어 넣으면 크롬 오류 화면이 남는다. 그건 사용자
